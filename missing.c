...
void
missing_arrays_add_column (gint jvar, datad *d, ggobid *gg)
{
  gint nc = d->ncols + 1;

  if (d->nmissing > 0) {
    arrays_add_cols (&d->missing, nc);
    arrayl_add_cols (&d->missing_jitter, nc);
    arrayl_add_cols (&d->missing_world, nc);
  }
}
