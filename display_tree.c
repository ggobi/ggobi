...
void
update_display_tree_plots_by_variable(DisplayTree * tree, gint whichVar,
                                      ...
                                      GtkWidget * w)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    tmp =
        (GtkWidget *)
        g_list_nth_data(gtk_container_children(GTK_CONTAINER(tmp)), 0);
...
GtkWidget *display_add_tree(displayd * display, gint entry,
                            GtkWidget * tree, ggobid * gg)
{
...
*/
  ...
  for (slist = display->splots; slist; slist = slist->next, ctr++) {
    ...
    gtk_signal_connect(GTK_OBJECT(item), "select",
                       GTK_SIGNAL_FUNC(display_tree_splot_child_select),
                       sp);
    gtk_widget_show(item);
  ...
  if (val) {
    tmp =
        g_malloc(sizeof(gchar *) *
                 (strlen(val) + strlen(display->d->name + 3 + 1)));
...
/*
  ...
  type `type'.
 */
gchar *splot_tree_label(splotd * splot, gint ctr, enum displaytyped type,
                        datad * d, ggobid * gg)
{
  gchar *buf = "";
  gint n;
  ...
  if (GTK_IS_GGOBI_EXTENDED_SPLOT(splot)) {
    return (GTK_GGOBI_EXTENDED_SPLOT_CLASS(GTK_OBJECT(splot)->klass)->
 ...
 */
gint
tree_display_entry_remove(displayd * display, GtkWidget * tree,
                          ggobid * gg)
{
  GList *dlist;
  displayd *tmp;
...
/*
 ...
 one.
 */
void display_tree_splot_child_select(GtkWidget * item, splotd * plot)
{
