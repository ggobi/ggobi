...
static void initRadialLayout (ggobid *gg, glayoutd *gl, datad *d, datad *e);
...
/*-- This may not belong here, but where exactly?  As soon as the
  ...
  init = (gl->radial == NULL || d->nrows != gl->radial->d->nrows);
  initRadialLayout (gg, gl, d, e);
...
*/
static void
initRadialLayout (ggobid *gg, glayoutd *gl, datad *d, datad *e) {
  gint i, nn;
  noded *na, *nb;
  gint nnodes = d->nrows;
  ...
  if (gl->radial != NULL) {
    ...
    gl->radial->nodes = (noded *) g_malloc (nnodes * sizeof (noded));
  }
  gl->radial->d = d;
  ...
  for (i = 0; i <e->edge.n; i++) {
    if (e->sampled.els[i] && !e->hidden.els[i]) {
      ...
      {
        na = &gl->radial->nodes[a];
        nb = &gl->radial->nodes[b];
