...
/*-- add variables generated by the layout algorithm --*/
  {
    ...
    for (i=0; i<d->nrows; i++) {
      ...
      depth[i] = (gdouble) ggv->radial->nodes[i].nStepsToCenter;
      inDegree[i] = (gdouble) g_list_length (ggv->radial->nodes[i].inEdges);
      outDegree[i] = (gdouble) g_list_length (ggv->radial->nodes[i].outEdges);
/*

For some reason, 
  g_list_length (ggv->radial->nodes[i].edges),
equals 0 every time.
*/
...
setNStepsToCenter (noded *n, noded *prevNeighbor) {
  ...
  GList *l;
  GList *connectedNodes = NULL;

  for (l = n->srcNodes; l; l = l->next) {
    n1 = (noded *) l->data;
    if (g_list_index (connectedNodes, n1) == -1)
      connectedNodes = g_list_append (connectedNodes, n1);
  }
  for (l = n->destNodes; l; l = l->next) {
    n1 = (noded *) l->data;
    if (g_list_index (connectedNodes, n1) == -1)
      connectedNodes = g_list_append (connectedNodes, n1);
  }
