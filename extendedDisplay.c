...
int addDisplayType(GtkType type)
{
  GtkObjectClass *klass;
  if (!gtk_type_is_a(type, GTK_TYPE_GGOBI_EXTENDED_DISPLAY)) {
    g_printerr("%s is not a Gtk type that extends GtkGGobiExtendedDisplay",
               gtk_type_name(type));
  }
...
void registerDisplayTypes(const GtkTypeLoad * const loaders, int n)
{
  int i;
  GtkType type;
