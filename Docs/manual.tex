...
\ref{fig5}). Finally, use these controls to hide the cases from
...
depending on the menu setting.  The brushing style called ``Hide'' 
deserves a few words: When it is selected, the brushed points are drawn
in a kind of shadow color, very close to the background color.  The
...
and for each row there are three buttons:  {\bf H} is a toggle button
that ``hides'' or shows all points drawn in the chosen symbol.  The three
remaining columns report the number of cases ``hidden,'' the number
shown, and the sum.  The word ``hide'' is enclosed in quotation marks
because ``hidden'' points are actually drawn in a color that's very
...
The {\bf Exclude hidden} button at the bottom of the window will exclude
all hidden points from consideration in the displays:  the views will be
scaled without those points, and they won't be drawn.  The {\bf Include
hidden} button will bring those points back into the plot, and they'll
be drawn in the shadow color as before.
