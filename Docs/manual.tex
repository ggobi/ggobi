...
The default linking rule is to link by case id: that is, points
...
events.  When no id has been provided, the record number becomes the
case id by default.

The second rule is to {\it Link by variable}, which uses the levels of
a categorical variable to link points.  The candidate variables, if
any, are named in the list widget under the {\em Undo} button.
Now when a case is brushed in one display, all cases with the same
value of the categorical variable will change accordingly, in this and
all other displays.
