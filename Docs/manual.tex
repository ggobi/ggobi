...
The data is interesting because it represents a quality control study
of olive oils. It is proposed that the olive oils from different
regions have different fatty acid signatures. 
...
Using the ViewMode menu, choose {\bf Identify}. Look at the
buttons inside the leftmost portion of the ggobi console. Notice that
they're contained by a frame labeled {\bf Identify,} which is the view
mode of the current plot.  This frame contains most of the row
...
written. The labels for this data set show the geographic area where
the sample was taken.

Using the Display menu, open a barchart display. Notice that
...
Now switch into {\bf 2D Tour} using the {\bf ViewMode} menu. A column of
...
touring, and toggle Region and Area out of the tour.  Then click {\bf
Reinit}.  The tour should now include 7 variables, with one variable
circle for each.
...
\item Rotation: 2-D tour constrained to the use of 3 variables,
...
\item the record id (if none was supplied, the record number is used).
\end{itemize}
...
things may happen; ggobi will attempt to catch this and warn you,
...
\item Graph layout  (implemented as a plugin)
\end{itemize}
