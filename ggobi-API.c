...
const gchar *GGOBI(setFileName) (const gchar * fileName,
                                 DataMode data_mode, ggobid * gg) {
...
gchar **GGOBI(getVariableNames) (gint transformed, datad * d, ggobid * gg) {
  gchar **names;
  gint nc = d->ncols, j;
  vartabled *vt;
...
void GGOBI(data_release) (datad * d, ggobid * gg) {
...
displayd *GGOBI(newScatterplot) (gint ix, gint iy, datad * d, ggobid * gg) {
...
displayd *GGOBI(newParCoords) (gint * vars, gint numVars, datad * d,
                               ggobid * gg) {
  displayd *display = NULL;
...
displayd *GGOBI(newTimeSeries) (gint * yvars, gint numVars, datad * d,
                                ggobid * gg) {
  displayd *display = NULL;
...
/*
  ...
  Don't touch this.
 */
const gfloat **GGOBI(getRawData) (datad * d, ggobid * gg) {
...
/*
  ...
  Don't touch this.
 */
const gfloat **GGOBI(getTFormData) (datad * d, ggobid * gg) {
 ...
 */
const gchar **GGOBI(getCaseNames) (datad * d, ggobid * gg) {
 ...
 */
void
GGOBI(setCaseName) (gint index, const gchar * label, datad * d,
                    ggobid * gg) {
  gchar *old;
...
gint *GGOBI(getCaseGlyphTypes) (gint * ids, gint n, datad * d, ggobid * gg) {
  gint i;
...
gint GGOBI(getCaseGlyphType) (gint id, datad * d, ggobid * gg) {
  gint index = d->rows_in_plot[id];
  return (d->glyph_now.els[index].type);
}

gint *GGOBI(getCaseGlyphSizes) (gint * ids, gint n, datad * d, ggobid * gg) {
  gint i;
  gint *ans = (gint *) g_malloc(n * sizeof(gint));
...
gint GGOBI(getCaseGlyphSize) (gint id, datad * d, ggobid * gg) {
  gint index = d->rows_in_plot[id];
...
void
GGOBI(setCaseGlyph) (gint index, gint type, gint size, datad * d,
                     ggobid * gg) {
  if (size > -1)
...
void GGOBI(setCaseColor) (gint pt, gint colorIndex, datad * d, ggobid * gg) {
...
gint GGOBI(getCaseColor) (gint pt, datad * d, ggobid * gg) {
  return (d->color_now.els[pt]);
}

gint *GGOBI(getCaseColors) (gint * pts, gint howMany, datad * d,
                            ggobid * gg) {
  gint i;
...
gboolean GGOBI(getCaseHidden) (gint pt, datad * d, ggobid * gg) {
  return (d->hidden_now.els[pt]);
}

gboolean *GGOBI(getCaseHiddens) (gint * pts, gint howMany, datad * d,
                                 ggobid * gg) {
  gint i;
...
gboolean GGOBI(isConnectedEdge) (gint a, gint b, datad * d, ggobid * gg) {
  gint tmp, i;
 ...
 */
void
GGOBI(setObservationEdge) (gint x, gint y, datad * d, ggobid * gg,
                           gboolean update) {
...
void redraw(splotd * sp, ggobid * gg)
{
...
gint GGOBI(setMode) (const gchar * name, ggobid * gg) {
...
gint GGOBI(getModeId) (const gchar * name) {
  gint n, i;
...
const gchar *GGOBI(getModeName) (int which) {
  int n;
  const gchar *const *names = GGOBI(getOpModeNames) (&n);
  return (names[which]);
}
...
/*    sprintf(rnames[i],"%d",i+1);*/
    ...
    /* May want the final false here to be true as it causes the 
       ...
       so we don't forget.
     */
    GGOBI(setData) (vals, rnames, &name, num, 1, d, false, gg, NULL,
                    d->input);
    for (i = 0; i < num; i++)
      g_free(rnames[i]);
    g_free(rnames);
...
void GGOBI(update_data) (datad * d, ggobid * gg) {
...
gint GGOBI(removeVariable) (gchar * name, datad * d, ggobid * gg) {
...
gint GGOBI(removeVariableByIndex) (gint which, datad * d, ggobid * gg) {
  gint i, j;
  for (i = 0; i < d->nrows; i++) {
...
gint GGOBI(getVariableIndex) (const gchar * name, datad * d, ggobid * gg) {
  gint j;
  vartabled *vt;
...
gint GGOBI(ncols) (datad * data) {
  return (data->ncols);
}

gint GGOBI(nrecords) (datad * data) {
  return (data->nrows);
}
 ...
 */
KeyEventHandler
    *GGOBI(registerNumberedKeyEventHandler) (KeyEventHandlerFunc routine,
                                             void *userData,
                                             char *description,
...
char *const GGOBI(getVersionDate) () {
  return (version_date);
}

char *const GGOBI(getVersionString) () {
  return (version_string);
}

int *const GGOBI(getVersionNumbers) () {
  return (GgobiVersionNumbers);
}


datad *GGOBI(data_get) (gint which, const ggobid * const gg) {
  datad *data = NULL;
...
datad *GGOBI(data_get_by_name) (const gchar * const name,
