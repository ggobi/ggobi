...
*/
  ...
  gtk_widget_set_tooltip_text ((tgl), gg->tips ? ("Cycle through 1D plots") : NULL);
  ...
  sbar = gtk_scale_new (GTK_ORIENTATION_HORIZONTAL, GTK_ADJUSTMENT (gg->p1d.cycle_delay_adj));
  gtk_widget_set_tooltip_text ((sbar), gg->tips ? ("Adjust cycling speed") : NULL);
  ...
  gtk_widget_set_tooltip_text ((btn), gg->tips ? ("Change cycling direction") : NULL);
