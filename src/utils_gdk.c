...
{
  cairo_surface_t *cairo_surface = g_object_get_data(G_OBJECT(drawable), "glitz_surface");
  ...
  if (!cairo_surface) {
    ...
    glitz_glx_init(NULL);
    {
      ...
      glitz_drawable_format_t *d_format = glitz_glx_find_drawable_format_for_visual (
    ...
    g_object_set_data_full (G_OBJECT (drawable), "glitz_surface", 
      cairo_glitz_surface_create (glitz_s), (GDestroyNotify)cairo_surface_destroy);
  }
