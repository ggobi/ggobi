...
To render on a display, data must be flattened into 2d.  There are two principle ways of doing this in GGobi: selection and projection.  Selection reduces the number of columns in the data to 1 or 2, while projection multiplies the data matrix by an orthonormal projection matrix with 1 or 2 columns.  (Selection is a special case of projection but is implemented separately for performance reasons)
