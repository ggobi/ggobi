...
ggobi_noop_toggle_button_get_type (void)
{
  ...
  if (!ggobi_noop_toggle_button_type) {
    ...
    ggobi_noop_toggle_button_type =
      g_type_register_static (GTK_TYPE_TOGGLE_BUTTON, "GGobiNoopToggleButton",
...
ggobi_noop_toggle_button_new (void)
{
  /* Do initialization up in _init(), not here.  If you need to
     ...
     before returning it. */
  return
    GTK_WIDGET (g_object_new (ggobi_noop_toggle_button_get_type (), NULL));
}
