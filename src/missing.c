...
missings_datad_cb (GtkWidget * w, ggobid * gg)
{
  ...
  ggobi_stage_set_name(dnew, g_strdup_printf ("%s (missing)", ggobi_stage_get_name(d)));
  ...
  for (i = 0; i < d->n_rows; i++) {
    ...
    ggobi_stage_set_attr_glyph(dnew, i, ggobi_stage_get_attr_glyph(d, i), ATTR_SET_PERSISTENT);
