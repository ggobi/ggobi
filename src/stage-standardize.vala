...
In the absence of information to the contrary, it is important to ensure that all variables are on a common scale.  In GGobi2 this involves rescaling each variable to [0, 1] (`tform_to_world`).  However, there are some cases when that is incorrect, for example if we know some variables were measured on the same scale.
