...
static void
selectXVar (GtkWidget * w, displayd * display, gint jvar, GGobiSession * gg)
{
...
static gboolean
varcircleDraw (displayd * display, gint jvar, GdkPixmap * da_pix, GGobiSession * gg)
{
...
static void
tourCorrRealloc (displayd * dsp, gint nc, GGobiStage * d)
{
  /*
...
static void
tour2d3Realloc (displayd * dsp, gint nc, GGobiStage * d)
{
  gint old_ncols, i;
  /*
...
static void
tour2dRealloc (displayd * dsp, gint nc, GGobiStage * d)
{
  gint old_ncols, i;
  /*
   * because display_tour2d_init_null has been performed even if
...
static void
tour1dRealloc (displayd * dsp, gint nc, GGobiStage * d)
{
  gint old_ncols, i;
  /*
...
static void
pmodeSet (ProjectionMode pmode, displayd * display, GGobiSession * gg)
{
...
displayd *
gtk_scatterplot_new (GGobiStage * d, GGobiSession * gg)
{
  displayd *display;
...
cpanelSet (displayd * dpy, cpaneld * cpanel, GGobiSession * gg)
{
  ...
  if (dpy->d->n_cols >= MIN_NVARS_FOR_TOUR2D3)
    cpanel_tour2d3_set (dpy, cpanel, gg);
  if (dpy->d->n_cols >= MIN_NVARS_FOR_TOUR2D)
    cpanel_tour2d_set (dpy, cpanel, gg);
  if (dpy->d->n_cols >= MIN_NVARS_FOR_COTOUR)
...
void
displaySet (displayd * dpy, GGobiSession * gg)
{
}
...
static void
varpanelTooltipsReset (displayd * display, GGobiSession * gg, GtkWidget * wx,
...
treeLabel (splotd * splot, GGobiStage * d, GGobiSession * gg)
{
  ...
  case P1PLOT:
  case TOUR1D:
    buf = ggobi_stage_get_col_name(d, splot->p1dvar);
    break;
...
static void
subPlaneToScreen (splotd * sp, displayd * dpy, GGobiStage * d, GGobiSession * gg)
{
...
static gboolean
drawCase (splotd * sp, gint m, GGobiStage * d, GGobiSession * gg)
{
  ...
  case P1PLOT:
    ...
    break;
  case XYPLOT:
    if (ggobi_stage_is_missing(d, m, sp->xyvars.x))
      draw_case = false;
    else if (ggobi_stage_is_missing(d, m, sp->xyvars.y))
      draw_case = false;
    break;
  case TOUR1D:
    for (j = 0; j < display->t1d.nactive; j++) {
      if (ggobi_stage_is_missing(d, m, display->t1d.active_vars.els[j])) {
        draw_case = false;
        break;
      }
    }
  ...
  case TOUR2D3:
    for (j = 0; j < display->t2d3.nactive; j++) {
      if (ggobi_stage_is_missing(d, m, display->t2d3.active_vars.els[j])) {
        draw_case = false;
        break;
      }
    }
  ...
  case TOUR2D:
    for (j = 0; j < display->t2d.nactive; j++) {
      if (ggobi_stage_is_missing(d, m, display->t2d.active_vars.els[j])) {
        draw_case = false;
        break;
      }
    }
  ...
  case COTOUR:
    for (j = 0; j < display->tcorr1.nactive; j++) {
      if (ggobi_stage_is_missing(d, m, display->tcorr1.active_vars.els[j])) {
        draw_case = false;
        break;
      }
    }
    if (draw_case) {
      for (j = 0; j < display->tcorr2.nactive; j++) {
        if (ggobi_stage_is_missing(d, m, display->tcorr2.active_vars.els[j])) {
          draw_case = false;
          break;
        }
      }
...
static gboolean
drawEdge (splotd * sp, gint m, GGobiStage * d, GGobiStage * e, GGobiSession * gg)
{
  displayd *display = sp->displayptr;
  ...
  case P1PLOT:
    ...
    break;
  case XYPLOT:
    if (ggobi_stage_is_missing(e, m, sp->xyvars.x))
      draw_edge = false;
    else if (ggobi_stage_is_missing(e, m, sp->xyvars.y))
      draw_edge = false;
    break;
  case TOUR1D:
    if (ggobi_stage_is_missing(e, m, sp->displayptr->t1d.active_vars.els[m]))
      draw_edge = false;
    break;

  case TOUR2D3:
    if (ggobi_stage_is_missing(e, m, sp->displayptr->t2d3.active_vars.els[m]))
      draw_edge = false;
    break;
  case TOUR2D:
    if (ggobi_stage_is_missing(e, m, sp->displayptr->t2d.active_vars.els[m]))
      draw_edge = false;
    break;

  case COTOUR:
    if (ggobi_stage_is_missing(e, m, sp->displayptr->tcorr1.active_vars.els[m]))
      draw_edge = false;
    else if (ggobi_stage_is_missing(e, m, sp->displayptr->tcorr2.active_vars.els[m]))
      draw_edge = false;
    break;
  case NULL_PMODE:
  case DEFAULT_PMODE:
...
void
addMarkupCues (splotd * sp, GdkDrawable * drawable, GGobiSession * gg)
{
...
void
addScalingCues (splotd * sp, GdkDrawable * drawable, GGobiSession * gg)
{
...
*/
  ...
  switch (cpanel->pmode) {
  case P1PLOT:
    var = ggobi_stage_get_variable(d, sp->p1dvar);
  ...
  case XYPLOT:
    /* x */
    var = ggobi_stage_get_variable(d, sp->xyvars.x);
    ggobi_variable_get_limits(var, &min, &max);
    rdiff = max - min;
    world.x = planar.x;
    ftmp = world.x ;
