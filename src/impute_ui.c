...
impute_window_open (ggobid * gg)
{
  ...
  if (gg->impute.window == NULL) {
    ...
    gtk_widget_set_tooltip_text ((tgl), gg->tips ? ("Draw the missing values when plotting displays") : NULL);
    ...
    g_signal_connect (G_OBJECT (btn),
                      ...
                      (gpointer) gg);
    gtk_widget_set_tooltip_text ((btn), gg->tips ? ("Generate a new dataset from the 1's and 0's representing missingness") : NULL);
    ...
    gtk_widget_set_tooltip_text ((radio), gg->tips ? ("Assign to each missing value one of the existing variable values chosen at random") : NULL);
   ...
   /*-- hbox to hold a few buttons --*/
    ...
    gtk_widget_set_tooltip_text ((btn), gg->tips ? ("Impute or assign values to missings") : NULL);
    gtk_box_pack_start (GTK_BOX (hb), btn, true, true, 2);

    btn = gtk_button_new_with_mnemonic ("Re_scale");
    gtk_widget_set_tooltip_text ((btn), gg->tips ? ("Rescale the data after imputing") : NULL);
