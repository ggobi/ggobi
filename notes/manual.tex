...
{\bf pigs.xml}, a dataset on pig production in the United Kingdom,
...
a plot of YEAR vs TIME.  Move the mouse to one of the variable labels
...
the 2d plots:  PROFIT vs HERDSZ, HERDSZ vs TIME.
...
scatterplot of the PROFIT vs GILTS, from the pig production data.
}}
\vspace{.4in}
}
...
Using the ViewMode menu, choose {\bf Brush.}  
...
labelled {\bf Brush,} which is the ViewMode of the current plot.  This
...
The pink rectangle
that appears in the current plot is the ``paintbrush,'' and dragging it
...
a central part of the ggobi graphical user interface.  It contains a
...
you can select one row of the table at a time, or use the control and
shift keys as modifiers to select more than one.  Hold the
