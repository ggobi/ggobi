...
the fatty acid variables, and choose {\bf Standardize} in the stage 2
transformation panel to standardize all the variables to have mean 0
and variance 1. The transformation tool has numerous transformation
operations to perform on variables as appropriate. Stage 0 is a
domain adjustment operation to adjust variables values into an
acceptable range as needed for some stage 1 transformations. Stage 2
...
statistics of variables. It is also used for variable subset selection
for launching multi-plot displays.
}%}
\label{fig4}
\end{figure}

Now look at the plot of linoleic vs eicosenoic. Open up the {\bf Color
\& glyph groups ...} panel, and use the Hide/Show chack boxes to toggle
...
select more than one.  Hold the control key down while clicking to
select variables that are not contiguous in the table; use the shift
key if you want to select a contiguous range of variables. Select all
...
Now select the first plot in the parallel coordinates display by
clicking on it, and using the {\bf ViewMode} menu switch to {\bf
...
Using the {\bf Color \& glyph groups} panel from the {\bf Brush}
controls, click on the appropriate `S' button to bring the Region 1
cases back into the plot. Now using the {\bf File} menu, output the
data, saving the colors and glyphs that have been created during this
session.
...
corresponds to the control panel?  If you select the {\bf Scale} mode,
...
\item Insert (Append):  First select one of the plots along the
  ...
  variable, then click on one of the variable checkboxes.  (GGobi will
...
\item Insert (Append):  First select one of the plots, then click on
  one of the variable checkboxes to insert (append) a new plot before
  (after) the current plot.  (GGobi will not add a variable that's
  already plotted.)
\item Delete:  No plot selection is required; just click on the
...
\item Insert (Append):  Select a plot in the display, and then click
  ...
  (after) the current plot.  (This applies to vertically plotted
  variables only.)
...
The basics of the ascii data format used in xgobi are still
supported, with some changes.
...
Circles, which may be the most visually appealing glyph, take much
longer to draw under Microsoft Windows.  Consequently, our sample
data files usually use a small filled rectangle as the default
glyph.
...
The textured dot plot uses a method (and code) described in
...
to the projection of the first active variable. 
...
A rudimentary guided tour is available when the {\bf Projection
Pursuit} button is selected. It is controlled through a separate
pop-up projection pursuit window. Currently only the principal
component index is implemented.
...
A rudimentary guided tour is available from the projection pursuit
control panel. Currently only the holes and central mass indices are
implemented. Projection pursuit in the 2D tour requires the variables
to be sphered ahead of time. So the user needs to first enter the {\bf
Tools} menu and use the {\bf Sphering...} tool to clone sphered
counterparts of the currently active variables. Once this is done then
the {\bf Projection Pursuit} option can be chosen. This pops up a new
window which provides an interface to guiding the tour.
...
zooming the camera in and out -- and the aspect ratio changes, too.
...
%
If {\bf Point brushing} is in any state but `Off,' the brush has a
...
Similarly, if {\bf Edge brushing} is in any state but `Off,' the
...
First specify the variables you wish to jitter.
...
The ``Brush by variable'' tool is used to automatically color points
using the color scale.  To use it, first select a variable.  This
adds two sets of numbers to the display:  along the bottom of the
display, at the center of each stripe of color, is shown the number
of points that will be drawn with this color once ``Apply'' is
pressed.  Along the top of the display, at the boundaries between
stripes of color, appears the value of the chosen variable that 
defines the boundary.

To adjust the boundaries, grab one of the sliders, and notice that
both the values and the counts adjust as you move it.

[We plan to support multiple color scales of different sizes,
and then this tool will become increasingly useful.]

\subsection{Color \& glyph groups}
\label{slbl:ColorAndGlyphGroups}

The `Color \& glyph groups' tool displays a table.  Each unique
symbol in the data (combination of color and clyph) occupies a row,
and for each row there are three buttons:  {\bf H} hides all points
drawn in the chosen symbol, {\bf S} shows them all, and {\bf C} hides
...
displays.  The {\bf Rescale} button will cause the views to be
redrawn as if any hidden points are no longer in the data:  that is,
the points that are shown will be scaled to fill the viewing area of
the plot.
...
the numerical representation of the level.)  [This has not yet been
...
the details of their format is described in (reference)
...
select variables -- not for specifying variable subsets when launching
a parallel coordinates or scatterplot matrix display.
