...
We begin with a tutorial, and move on to describe ggobi's layout
...
taken from {\em Data} by Andrews and Herzberg.  Start ggobi for the 
pig production data by typing:
...
{Figure~1: Layout of a ggobi session.  The plotting window contains a
scatterplot of the pig production data.
}}
\vspace{.4in}
}
...
Across the top of the control panel in Figure 1 stretches a row of
buttons for selecting major GGobi functions from pull-down menus.
Buttons such as {\bf File}, {\bf Display,} {\bf ViewMode}, {\bf
Tools}, {\bf Options} and {\bf DisplayTree} organize functions in
menus.
...
is discussed in section ~\label{DisplayTypes}.
...
\item Move Points: direct manipulation of point position.
\end{itemize}

Each of these operations defines the mouse operations for directly
manipulating the view.  Examples are dragging the mouse to control
point cloud rotations, or to highlight points.  These operations also
determine the content of the control panel on the left of the
window.  Exactly one mode is selected at all times.  The view modes
...
\subsection{Brush: brushing of points and lines}
\label{slbl:Brush}
...
in the chosen color.  (There is no line brushing implementation for
line types yet.) If both {\bf Points} and {\bf Edges} are selected,
...
neat definitions.  Stage 1 transformations include the Box-Cox
transformation, and you can either type the Box-Cox parameter into
the text box and hit return, or use the spin button to gradually
increase or decrease the parameter.  Many of the stage 2
...
date.  For instance, it's no longer possible to specify line segments
...
points to line segments.  This has been replaced with a single set
