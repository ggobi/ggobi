...
static void
init_file_rows_sampled (ggobid *gg) {
  gint i, k, t, m, n;
  gfloat rrand;
...
static gboolean
seek_to_file_row (gint array_row, FILE *fp, ggobid *gg) {
  gint i, ch, file_row;
  ...
  while (true) {
    if (jcols == 0) {
      if (gg->file_read_type == read_all) {
        ...
        if (!seek_to_file_row (nrows, fp, gg))
          break;
      }
    }
