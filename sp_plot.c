...
/*-------------------------------------------------------------------------*/
/*---------         routines for writing foo.svg         ------------------*/
/*-------------------------------------------------------------------------*/

gchar *colorhex (GdkColor *color) {
  gulong val = 0;
  gchar buf[32];
...
svg_write_header (FILE *f) {
 ...
 fprintf (f, "<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 20000303 Stylable//EN\"\n");
 fprintf (f, "\"http://www.w3.org/TR/2000/03/WD-SVG-20000303/DTD/svg-20000303-exchange.dtd\">\n");
 fprintf (f, "\n");
}
...
splot_write_svg (splotd *sp, ggobid *gg)
{
  ...
  svg_write_header (f);
/*-- don't need a viewBox if we're going to write out sp->screen values --*/
/*
  fprintf (f,
    "<svg width=\"%dpx\" height=\"%dpx\">\n",
    da->allocation.width, da->allocation.height);
*/
/*-- use a viewBox if we're going to write out the values in d->tform2 --*/
  fprintf (f,
    ...
    for (k=0; k<npoint_colors_used; k++) {
      ...
      for (i=0; i<d->nrows_in_plot; i++) {
        ...
        if (draw_case && d->color_now[m] == current_color) {
          if (display->options.points_show_p) {
            gchar *hexcolor =
              colorhex (&gg->default_color_table[current_color]);
...
*/
/*-- write out values in d->tform2 so R can build axes --*/
            fprintf (f, "cx=\"%f\" cy=\"%f\" r=\"%d\"/>\n",
              d->tform2.vals[m][sp->xyvars.x],
              d->tform2.vals[m][sp->xyvars.y], 5);
          }
        }
      }
#endif
