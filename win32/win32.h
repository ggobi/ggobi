 ...
 *
 * NOTE: If called on a directory the values in the time fields are not
only
...
int	_fstat (int nHandle, struct _stat* pstat);
int	_chmod (const char* szPath, int nMode);
int	_stat (const char* szPath, struct _stat* pstat);
